...
def answer_one():
    ...
    new_x = np.linspace(0,10,100).reshape(100,1)
    predictions_array = np.empty((0,len(new_x)))

    for i in [1, 3, 6, 9]:
        poly_transform = PolynomialFeatures(degree=i)
        X_poly = poly_transform.fit_transform(x_train)
        fitted_model = LinearRegression().fit(X_poly, y_train)    

        transformed_x_test = poly_transform.fit_transform(new_x)
        predicted_model = fitted_model.predict(transformed_x_test).reshape(1,100)
        predictions_array = np.append(predictions_array, predicted_model,axis=0)
    
    
    return predictions_array # Return your answer
...
def answer_four():
    ...
    poly_transform = PolynomialFeatures(degree=12).fit(x_train)
    X_train_poly = poly_transform.transform(x_train)
    X_test_poly = poly_transform.transform(x_test)    
    ...
    linear_model_prediction = linear_model.predict(X_test_poly)
    LinearRegression_R2_test_score = r2_score(y_test, linear_model_prediction)

    lasso_model_prediction = lasso_model.predict(X_test_poly)
    Lasso_R2_test_score = r2_score(y_test, lasso_model_prediction)
...
def answer_six():
    ...
    train_scores, test_scores = validation_curve(model, X_subset, y_subset, param_name='gamma', param_range=param_range)
